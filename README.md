...
3. **运行**（两个 Agent 独立使用；公共底座命令见下）：
   ...
   - 确实想把历史简报声明为已发送，必须显式 `--mark-existing-sent --confirm`，
     且**只标记截止点之前**的那些，晚于截止点的照常推送；拿不准就先加 `--dry-run`
     看一遍会标记什么。备份写失败时该命令会中止并报错，不会动原状态。
...
5. **定时（可选）**：`crontab -e` 添加，例如每天早上 9 点（`&&` 保证某步失败即停，
   ...
   - **处理成功才允许推送**：`ads run` 逐封记录 `ready` / `empty` / `retryable_error`；
     ADS API 或 LLM 失败（含返回空内容、缺标题）时该邮件**不会**被记成已处理，
     下次运行自动重试（不是空话）。处理状态一旦建立，候选就只从 `ready` 产生——
     不会退回"扫描磁盘上的简报"而把半成品发出去。
   - **断言"没有新邮件"需要完整证据**：检索成功、无拉取缺口、接管覆盖完整、
     且拉取时间晚于截止点，四条同时成立才会说"今日无新推送"；否则状态邮件会
     如实说明本轮没能完整检查邮箱。
   - **拉取是增量的**：按 UID 游标只取新邮件，失败的邮件进入缺口队列持续重试；
     首次接管默认全量（上限 `MAIL_DIGEST_FETCH_INITIAL_MAX`，超出会分批补拉，不会静默丢弃）。
   - 停机几天后恢复会把积压简报补齐（无 3 天窗口限制）；但每轮 `ads run` 默认最多处理
     20 封（`--limit` 可调），积压很多时会分几轮跑完，不会一次全塞进一封邮件。
     SMTP 发送失败时截止点不推进，下次自动重发同一批。
